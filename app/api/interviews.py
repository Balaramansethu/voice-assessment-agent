...
from app.api.schemas import AnswerRequest, IntentRequest
...
def get_interview(interview_id: int, session: Session = Depends(get_session)) -> dict:
...
def get_state(interview_id: int, session: Session = Depends(get_session)) -> dict:
    interview = _get(session, interview_id)
    return {
        "id": interview.id,
...
@router.post("/action")
def take_action(body: IntentRequest, session: Session = Depends(get_session)) -> dict:
...
def complete(interview_id: int, session: Session = Depends(get_session)) -> dict:
...
def interrupt(interview_id: int, session: Session = Depends(get_session)) -> dict:
...
def evaluate(interview_id: int, session: Session = Depends(get_session)) -> dict:
